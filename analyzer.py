...
def analyse(prog,befspace=0,flag=0,line=1):
    ...
    while flag<length:#从左向右处理
        if prog[flag]=="#":#不考虑头文件及预编译
            ...
            #     flag += 1
            #     if prog[flag]!='\n':
            #         print("< ERROR,STAY \ ,LINE "+str(line)+" "+str(flag-befspace)+">",end=' ')
            #         while prog[flag] != '\n':
            #             flag += 1
            #         befspace=flag
            #         line+=1
                    ...
                    if prog[flag].lower()=='u' :#判断是否有符号、长整，当前flag指向十六进制后一位
                        if prog[flag+1].lower()=='l':
                            if prog[flag + 2].isalnum():
                                ...
                                flag+=2
                        elif prog[flag+1].isalnum():#0x23fu2出错
            ...
            flag=skip(prog,flag)
        pass
def isdeli(ch) ->bool:#是否为（不可跳过字符）或（不是分界符）
    return ch!='\n' and not ch.isspace() \
           and not ch in delimiter and not ch in operetor

def next(prog ,flag,ignore=False):#判断传入串flag位置后一个属性
    start=flag+1
    if ignore:
